...
namespace ChatCore
{
    ...
    public class ChatService : IChatService
    {
        ...
        {
            using (SqlConnection connection = new SqlConnection("server=SKSIRIUSR-PC\\SQLSKSIRIUS;Trusted_connection=yes;database=DiagnosticsDB;connection timeout=30"))
            {
        ...
        public static bool Authenticate(string userName, string password)
        {
            using (SqlConnection connection = new SqlConnection("server=SKSIRIUSR-PC\\SQLSKSIRIUS;Trusted_connection=yes;database=DiagnosticsDB;connection timeout=30"))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                        if (userName == reader[0].ToString() && UnEncodePassword(password) == reader[1].ToString())
                            return true;
                    reader.Close();
                }
                catch (Exception ex)
        ...
        private static string UnEncodePassword(string Password)
        {
            SHA1CryptoServiceProvider m = new SHA1CryptoServiceProvider();

            string pass = Encoding.Unicode.GetString(m.ComputeHash(Convert.FromBase64String(Password)));
            Convert.From
            return pass;
        }
    }
